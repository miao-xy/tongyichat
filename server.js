...
app.use(express.static(join(__dirname, 'public'))); // 提供静态文件服务，默认目录为 public
console.log("中间层配置成功");

// 创建 OpenAI 实例，配置 API 密钥和基础 URL
const openai = new OpenAI({
    apiKey: "sk-f01ff4aabe8443e39c8b2a52a523ed2d",
...
app.post('/api/chat', async (req, res) => {
    ...
    try {
        ...
        console.log(`AI回复内容:${aiRole}:${aiResponse.substring(0, 50)}...`); // 打印 AI 的回复内容（截取前 2
        res.json({
            message: aiResponse,
            role: aiRole
        }); // 将 AI 的回复内容和角色作为 JSON 响应返回
